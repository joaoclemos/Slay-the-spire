 ...
 * renderer.c
 *
 * RESPONSABILIDADE: Cuidar de tudo o que aparece na tela.
 * - Carregar imagens do disco (HD) para a memória (RAM).
 * - Desenhar cartas, personagens, fundos e textos.
 * - Limpar a tela e mostrar o próximo quadro (frame).
 */

#include "renderer.h"
#include <allegro5/allegro_primitives.h> // Para desenhar formas (retângulos, círculos)
#include <allegro5/allegro_ttf.h>        // Para escrever textos com fontes
#include <stdio.h>
#include "utils.h"

// --- DESENHA ÍCONES DE STATUS ---
// Função auxiliar que desenha bolinhas coloridas embaixo do personagem
// para mostrar se ele tem Força, Veneno, etc.
void draw_status_icons(Renderer* r, Creature c, float x, float y) {
    float offset_x = 0; // Deslocamento para não desenhar um ícone em cima do outro
    
    // Se tiver Força, desenha bolinha Vermelha
    if (c.forca > 0) {
        al_draw_filled_circle(x + offset_x, y, 10, al_map_rgb(200, 50, 50));
        // Escreve "F: valor" dentro
        al_draw_textf(r->font, al_map_rgb(255,255,255), x + offset_x, y-5, ALLEGRO_ALIGN_CENTER, "F:%d", c.forca);
        offset_x += 25; // Move para o lado para o próximo ícone
    }
    // Se tiver Destreza, desenha bolinha Azul
    if (c.destreza > 0) {
    ...
    // Se tiver Veneno, desenha bolinha Verde
    if (c.veneno > 0) {
    ...
    // Se estiver Dormindo, desenha bolinha Azul Clara
    if (c.dormindo > 0) {
    ...
    // Se tiver Regeneração, desenha bolinha Rosa
    if (c.regeneracao > 0) {
...
// --- DESENHA UMA CRIATURA (Jogador ou Inimigo) ---
// Desenha: Base de Pedra -> Personagem -> Nome -> Barra de Vida -> Escudo -> Status
void draw_creature(Renderer* renderer, Creature creature, float x, float y, const char* label) {
    ...
    // (x, y) é o pé do personagem. Desenhamos (y - altura) para ele subir.
    // O +15 é um ajuste fino para o pé encostar na pedra.
    ...
    // 6. DESENHA O ESCUDO (Bolinha Azul ao lado da vida)
    ...
    // 7. CHAMA A FUNÇÃO DE ÍCONES DE STATUS
...
void draw_card(Renderer* r, Card c, float x, float y) {
    ...
    // 1. LÓGICA PARA ESCOLHER O ÍCONE E O TEXTO
    switch (c.tipo) {
        case ATAQUE:
            if (c.is_vampiric) {
    ...
    // 2. DESENHA O FUNDO (VERSO DA CARTA)
    ...
    // 3. DESENHA O ÍCONE NO CENTRO (LEVEMENTE ACIMA)
    if (img_icone) {
    ...
    // 4. DESENHA O TOKEN DE CUSTO (Canto Superior Esquerdo)
    float cx = x + 22; float cy = y + 22; 
    float radius = 11; 
    // Círculos concêntricos para fazer uma moeda dourada
    al_draw_filled_circle(cx + 2, cy + 2, radius, al_map_rgba(0, 0, 0, 100)); // Sombra
    al_draw_filled_circle(cx, cy, radius, al_map_rgb(218, 165, 32)); // Ouro
    al_draw_filled_circle(cx, cy, radius - 4, al_map_rgb(255, 255, 255)); // Branco
    al_draw_circle(cx, cy, radius - 4, al_map_rgb(139, 69, 19), 2); // Contorno
...
// --- DESENHA OS MONTES DE COMPRA/DESCARTE ---
void draw_deck_pile(Renderer* r, int x, int y, int qtd, const char* label) {
    if (qtd > 0) {
        if (r->img_verso) {
            // Desenha a imagem do verso várias vezes para parecer um monte 3D
...
// --- RENDERIZAÇÃO PRINCIPAL (ROTEADOR) ---
    ...
    if (state == GAME_STATE_VICTORY) {
        ...
        for(int i=0; i<2; i++) {
            if (inimigos[i].stats.hp_atual > 0) {
                ...
                // Desenha a Intenção (se está dormindo ou o que vai fazer)
                ...
                // Desenha a caixa vermelha de seleção (se estiver mirando)
                if (mirando && sel_e == i) {
        ...
        for(int i=0; i<p.mao.num_cartas; i++) {
            float y = HAND_Y;
            // Efeito de "levantar" a carta selecionada
            if (i == sel_c) {
                y -= 30; 
                // Desenha borda amarela de seleção atrás da carta
